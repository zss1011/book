...
const handleRoute = (menu) => {
    ...
    // 发布事件
    emitter_js_1.default.emit('breadcrumb', menu.meta.path);
};
// 设置组件name
defineOptions({
    name: 'SubMenu'
...
for (const [menu] of __VLS_getVForSourceType((menus))) {
    ...
    if (menu.children && menu.children.length) {
        ...
        const __VLS_28 = {}.SubMenu;
        /** @type {[typeof __VLS_components.SubMenu, ]} */ ;
        // @ts-ignore
...
const __VLS_self = (await Promise.resolve().then(() => __importStar(require('vue')))).defineComponent({
    setup() {
        return Object.assign(Object.assign({ $props: __VLS_makeOptional({ menus }) }, { menus }), { handleRoute: handleRoute });
    },
});
